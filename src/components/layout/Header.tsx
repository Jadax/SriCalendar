...
export function Header({ firstName, avatarUrl, onAvatarChange }: HeaderProps): ReactElement {
  ...
  return <><header className="topbar"><div className="greeting"><span className="season">{getSeasonalDecoration()}</span><div><h1>{getGreeting(undefined, firstName)}</h1><p>Make something wonderful today.</p></div></div><div className="top-actions"><div className="streak" title="Daily visit streak">🔥 <strong>{streak}</strong><span> {streak === 1 ? 'day' : 'days'}</span></div><SyncStatus/><ThemeToggle/>{user && <button className="icon-button" onClick={() => setVaultOpen(true)} aria-label="Open data vault"><HardDriveDownload size={18}/></button>}<input ref={inputRef} className="avatar-input" type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void chooseAvatar(event)} tabIndex={-1}/><button className={`avatar-button ${savingAvatar ? 'saving' : ''}`} onClick={() => inputRef.current?.click()} disabled={savingAvatar} aria-label="Upload or change profile photo" title="Change profile photo">{avatar ? <img className="avatar" src={avatar} alt="Your profile" /> : <span className="avatar fallback">{user?.email?.[0]?.toUpperCase() ?? 'S'}</span>}<span className="avatar-edit"><Camera size={13}/></span></button><button className="icon-button logout" onClick={() => void logout()} aria-label="Sign out"><LogOut size={18}/></button></div></header>{avatarError && <p className="avatar-error" role="status">{avatarError}</p>}{vaultOpen && user && <DataVault userId={user.id} onClose={() => setVaultOpen(false)}/>}</>;
}
