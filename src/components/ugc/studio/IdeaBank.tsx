...
import { fillTemplate, PLATFORMS, PRIORITIES, EFFORT_LEVELS, IDEA_STATUSES, PRIORITY_META, EFFORT_META, PLATFORM_META } from '../../../data/options';
  ...
  return <>
    ...
    <section className="section-block">
      <div className="board-toolbar">
        <FormRow>
          <Field label="Status"><select className="select" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}><option value="all">All statuses</option>{IDEA_STATUSES.map((s) => <option key={s}>{s}</option>)}</select></Field>
          <Field label="Platform"><select className="select" value={platformFilter} onChange={(e) => setPlatformFilter(e.target.value)}><option value="all">All platforms</option>{PLATFORMS.map((s) => <option key={s}>{s}</option>)}</select></Field>
          <Field label="Priority"><select className="select" value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value)}><option value="all">All priorities</option>{PRIORITIES.map((s) => <option key={s}>{s}</option>)}</select></Field>
          <Field label="Effort"><select className="select" value={effortFilter} onChange={(e) => setEffortFilter(e.target.value)}><option value="all">All effort</option>{EFFORT_LEVELS.map((s) => <option key={s}>{s}</option>)}</select></Field>
    ...
    {editing && <Modal title={editorId ? 'Edit idea' : 'New idea'} onClose={() => setEditing(null)} wide
      ...
      <div className="grid" style={{ gap: 14 }}>
        ...
        <FormRow>
          ...
          <Field label="Priority"><select className="select" value={editing.priority ?? 'medium'} onChange={(e) => setEditing({ ...editing, priority: e.target.value })}>{PRIORITIES.map((p) => <option key={p}>{p}</option>)}</select></Field>
          <Field label="Effort"><select className="select" value={editing.effort_level ?? 'quick'} onChange={(e) => setEditing({ ...editing, effort_level: e.target.value })}>{EFFORT_LEVELS.map((p) => <option key={p}>{p}</option>)}</select></Field>
        </FormRow>
        <Field label="Status"><select className="select" value={editing.status ?? 'idea'} onChange={(e) => setEditing({ ...editing, status: e.target.value })}>{IDEA_STATUSES.map((p) => <option key={p}>{p}</option>)}</select></Field>
        <FormRow>
